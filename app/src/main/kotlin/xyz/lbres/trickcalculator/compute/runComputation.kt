 ...
 * @param performSingleOp [OperatorFunction]: function for performing operations
 * @param numbersOrder [List]: list of numbers, which can be used to reassign the values of digits
 * @param applyParens [Boolean]: if parentheses should be recognized. If `false`, all parentheses will be removed before computation.
...
fun runComputation(
    ...
    return try {
        ...
        val error = getParseErrorMessage(e.message)
 ...
 * @param operatorRounds [List]<StringList>: list of string values recognized as operators, separated into "rounds" that should be applied in the same passthrough.
 * @param performSingleOp [OperatorFunction]: function for performing operations
 * @return [ExactFraction] the single computed value
 * @throws ArithmeticException in case of divide by zero
 ...
 * @param performSingleOp [OperatorFunction]: function for performing operations
 * @return [StringList] a modified list where all pairs of parentheses have been simplified to a single ExactFraction, represented as a EF string
 */
fun parseParens(
    computeText: StringList,
    operatorRounds: List<StringList>,
