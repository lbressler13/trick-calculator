...
class ExactFraction private constructor() : Comparable<ExactFraction>, Number() {
    ...
    operator fun unaryPlus(): ExactFraction =
        ExactFraction(numerator, denominator, fullSimplified = true)
    ...
    fun pow(other: ExactFraction, maxLog: BigInteger?): ExactFraction {
    ...
    fun pow(other: ExactFraction): ExactFraction = pow(other, null)
    ...
    private fun overflowException(type: String): Exception =
        ExactFractionOverflowException(
            "Overflow when casting to $type",
            toFractionString()
        )
